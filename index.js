...
function createMenuItem(name, cost, category) {
  const newItems = { name: name, price: cost, category: category };
...
createMenuItem("pie", 2, "desert");
createMenuItem("cheeseBurger", 20, "lunch");
createMenuItem("onionRings", 3, "side");
...
function getReviewByIndex(reViewArray, index) {
  return `${reViewArray[index].name} gave the restaurant a ${reViewArray[index].rating}, star review and their feedback was; ${reViewArray[index].feedback}`;
}
...
function getLastReview(reViewArray) {
  const lastReview = reViewArray.length - 1;
  return `${reViewArray[lastReview].name} gave the restaurant a ${reViewArray[lastReview].rating}, star review and their feedback was; ${reViewArray[lastReview].feedback}`; /* code here */
}
...
function getReviewByRating(objReview, movieRating) {
  // reviews.rating = movieRating;

  if (reviews.rating === movieRating) {
